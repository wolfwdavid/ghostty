...
pub fn add(
    ...
    // Statically compile glad (the GL loader) whenever we use the OpenGL
    // renderer. Needed for executables (GTK) and for libghostty alike: the
    // embedded/Qt apprt loads GL through glad on its own render thread.
    if (self.config.renderer == .opengl) {
